...
function storageKey(y, m) { return `spendlog_v2_${y}_${m}`; }
...
function loadAll() {
  ...
  for (let y = 2020; y <= 2035; y++) {
    for (let m = 0; m < 12; m++) {
      try {
        const d = JSON.parse(localStorage.getItem(storageKey(y, m)) || '[]');
        allExpenses = allExpenses.concat(d);
...
function saveExpense(exp) {
  const d = new Date(exp.ts);
  const y = d.getFullYear();
  const m = d.getMonth();
  let arr = [];
  try { arr = JSON.parse(localStorage.getItem(storageKey(y, m)) || '[]'); } catch (e) {}
  arr = arr.filter(e => e.id !== exp.id);
  arr.unshift(exp);
  localStorage.setItem(storageKey(y, m), JSON.stringify(arr));
}

function deleteFromStorage(exp) {
  const d = new Date(exp.ts);
  const y = d.getFullYear();
  const m = d.getMonth();
  let arr = [];
...
function buildFilterChips() {
  ...
  usedCats.forEach(c => {
    ...
    d.className = 'chip' + (activeFilter === c ? ' active' : '');
    d.textContent = c;
    d.onclick = function () { setFilter(c, this); };
    row.appendChild(d);
  });
}
...
function renderCharts() {
  ...
  const activeCats = Object.keys(catTotals)
  ...
  catChartInst = new Chart(document.getElementById('cat-chart').getContext('2d'), {
    ...
    data: {
      labels: activeCats,
      datasets: [{ data: activeCats.map(c => catTotals[c]), backgroundColor: aColors, borderWidth: 2, borderColor: '#ffffff' }]
    },
    options: {
      responsive: true,
  ...
  dailyChartInst = new Chart(document.getElementById('daily-chart').getContext('2d'), {
    ...
    data: {
      labels: dayLabels,
      datasets: [{ data: days.map(k => dailyMap[k]), backgroundColor: '#1D9E75', borderRadius: 3, borderSkipped: false }]
    },
    options: {
      responsive: true,
      maintainAspectRatio: false,
...
function renderTrends() {
  ...
  momChartInst = new Chart(document.getElementById('mom-chart').getContext('2d'), {
    ...
    data: {
      ...
      datasets: [{
        data: moTotals.map(m => m.total),
        backgroundColor: moTotals.map((_, i) => i === 5 ? '#1D9E75' : '#9FE1CB'),
        borderRadius: 4,
        borderSkipped: false
      }]
    },
...
function renderYearly() {
  ...
  yrChartInst = new Chart(document.getElementById('yr-chart').getContext('2d'), {
    ...
    data: {
      ...
      datasets: [{
        data: moTotals,
        backgroundColor: MONTHS.map((_, i) => i === thisM ? '#1D9E75' : '#9FE1CB'),
        borderRadius: 4,
        borderSkipped: false
      }]
    },
...
function importCSV(input) {
  ...
  reader.onload = e => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const id = fields[0] || Date.now() + Math.random();
