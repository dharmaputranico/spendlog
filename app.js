...
const I18N = {
  en: {
    ...
    cloudSync:'☁ Cloud sync', cloudDesc:'Data syncs automatically to Supabase. Open on any device and it\'s up to date.',
  ...
  id: {
    ...
    cloudSync:'☁ Sinkronisasi cloud', cloudDesc:'Data tersinkron otomatis ke Supabase. Buka di perangkat manapun.',
...
// Shared month cursor — all three navigable views stay in sync
const today = new Date();
let viewYear_m  = today.getFullYear();
let viewMonth_m = today.getMonth(); // 0-indexed

// Week cursor for Trends tab
...
function monthsShort() { return lang==='id' ? MONTHS_SHORT_ID : MONTHS_SHORT_EN; }

// ── MONTH / WEEK NAVIGATION ────────────────────────────────────────────────

function isCurrentMonth() {
  return viewYear_m === today.getFullYear() && viewMonth_m === today.getMonth();
}
...
function changeViewMonth(delta) {
  ...
  if (active) {
    const tab = active.getAttribute('data-i18n');
    if (tab==='tabLedger')    { buildFilterChips(); renderLedger(); }
    if (tab==='tabAnalytics') renderCharts();
    if (tab==='tabTrends')    renderTrends();
  }
  // Always update top metrics (they always show current-actual month, not view month)
  renderMetrics();
}

function changeViewWeek(delta) {
  viewWeekOffset += delta;
  refreshWeekNav();
  renderTrends();
}

// Returns expenses for the currently viewed month
...
// Returns week numbers present in the viewed month, sorted
function weeksInViewMonth() {
  const exp = viewMonthExp();
  return [...new Set(exp.map(e => getWeekNum(new Date(e.ts))))].sort((a,b)=>a-b);
}
...
function navMonthLabel() {
  ...
  const isCur = isCurrentMonth();
  return `<span>${label}</span>${isCur ? `<span class="nav-today-badge">${t('today')}</span>` : ''}`;
}

function refreshAllNavBars() {
  ['ledger-month-label','analytics-month-label','trends-month-label'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.innerHTML = navMonthLabel();
  });
...
function refreshWeekNav() {
  ...
  // Clamp viewWeekOffset to available weeks
  const maxOffset = -(weeks.length - 1);
  viewWeekOffset = Math.max(viewWeekOffset, maxOffset);
  viewWeekOffset = Math.min(viewWeekOffset, 0);

  const wkIdx   = weeks.length - 1 + viewWeekOffset; // 0 = oldest, last = newest
...
// Returns the selected week number for the Trends tab
function selectedWeekNum() {
  const weeks = weeksInViewMonth();
  if (!weeks.length) return null;
  const wkIdx = weeks.length - 1 + viewWeekOffset;
  return weeks[Math.max(0, wkIdx)];
}
...
function onCatChange() {
  ...
  document.getElementById('other-row').style.display = v === catKeyToEN('catOthers') ? 'block' : 'none';
}
...
async function addExpense() {
  ...
  btn.disabled = false;
  if (error) {
    console.error(error); setSyncStatus('error','saveFailed');
    showToast(t('toastSaveErr')); cacheAll();
  } else {
    setSyncStatus('synced','synced'); cacheAll(); showToast(t('toastSaved'));
  }
}
...
async function confirmDelete() {
  ...
  if (error) {
    console.error(error); setSyncStatus('error','deleteFailed');
    showToast(t('toastDeleteErr'));
    await loadFromSupabase(); renderLedger(); renderMetrics();
  } else {
    setSyncStatus('synced','synced'); cacheAll();
  }
}
...
function buildFilterChips() {
  const usedCats = [...new Set(viewMonthExp().map(e => e.cat))];
...
// ── RENDER METRICS (always shows real current month) ──────────────────────

function renderMetrics() {
  // Metrics always show the REAL current month, not the navigated view month
  const curExp   = allExpenses.filter(e => {
    const d = new Date(e.ts);
    return d.getFullYear()===today.getFullYear() && d.getMonth()===today.getMonth();
  });
  const total      = curExp.reduce((s,e)=>s+e.amount,0);
  const thisWk     = getWeekNum(today);
  const wkTotal    = curExp.filter(e=>getWeekNum(new Date(e.ts))===thisWk).reduce((s,e)=>s+e.amount,0);
  const prevWkTotal= curExp.filter(e=>getWeekNum(new Date(e.ts))===thisWk-1).reduce((s,e)=>s+e.amount,0);

  document.getElementById('m-total').textContent = fmtS(total);
  document.getElementById('m-count').textContent = `${curExp.length} ${t('entries')}`;
  ...
  const prevMo      = new Date(today.getFullYear(), today.getMonth()-1, 1);
  const prevMoTotal = allExpenses.filter(e=>{
    const d=new Date(e.ts);
    return d.getFullYear()===prevMo.getFullYear() && d.getMonth()===prevMo.getMonth();
  ...
  if (prevMoTotal > 0) {
    ...
    momEl.textContent='—'; momEl.className='mval neu';
    document.getElementById('m-mom-sub').textContent = t('vsLastMonth');
  }
}
...
function renderLedger() {
  ...
  if (monthly.length > 0) {
    totEl.innerHTML = `${t('monthTotal')}: <strong>${fmt(total)}</strong>`;
  } else {
    totEl.innerHTML = '';
  }

  if (!filtered.length) {
    tbody.innerHTML=''; empty.style.display=''; pag.style.display='none'; return;
  }
  ...
  if (filtered.length <= PAGE_SIZE) {
    ...
    btn.disabled     = shown >= filtered.length;
...
function renderCharts() {
  // Update daily-chart title to show which month
  document.getElementById('daily-chart-title').textContent =
    `${t('dailySpend')} — ${monthsLong()[viewMonth_m]} ${viewYear_m}`;
...
function renderTrends() {
  ...
  // Weekly bars (all weeks in the viewed month)
  document.getElementById('weekly-rows').innerHTML = weeks.length
    ? weeks.map(wk => {
        const isSelected = wk===selWk;
        const isCur      = isCurrentMonth() && wk===curWk;
        return `<div class="bar-row" style="${isSelected?'background:var(--accent-l);border-radius:6px;padding:2px 6px;margin-left:-6px;margin-right:-6px;':''}">
          <div class="bar-top">
            <span style="color:var(--muted)">${t('week')} ${wk}
              ...
              ${isSelected&&!isCur?`<span style="font-size:9px;background:var(--surf);color:var(--muted);padding:1px 5px;border-radius:99px;margin-left:4px">▶</span>`:''}
            </span>
            <span style="font-family:'DM Mono',monospace">${fmt(weekMap[wk])}</span>
          </div>
          <div class="bar-bg"><div class="bar-fill" style="width:${((weekMap[wk]/maxW)*100).toFixed(1)}%;background:${isSelected?'var(--accent)':'var(--hint)'}"></div></div>
        </div>`;
      }).join('')
    : `<div class="empty">${t('noData')}</div>`;

  // Selected week expenses
  ...
  // Previous month
  const prevMoDate   = new Date(viewYear_m, viewMonth_m-1, 1);
  const prevMoExp    = allExpenses.filter(e=>{
    const d=new Date(e.ts);
    return d.getFullYear()===prevMoDate.getFullYear() && d.getMonth()===prevMoDate.getMonth();
  });
  const prevMoTotal  = prevMoExp.reduce((s,e)=>s+e.amount,0);

  // Previous week (same week number, in prev week)
  ...
  const momPct = prevMoTotal>0 ? (monthTotal-prevMoTotal)/prevMoTotal*100 : null;

  const daysInMonth  = new Date(viewYear_m, viewMonth_m+1, 0).getDate();
  ...
  document.getElementById('mom-rows').innerHTML = `
    ...
    <div class="trow"><span class="tlabel">${t('avgPerDay')}</span><span class="tval">${daysElapsed?fmt(monthTotal/daysElapsed):'—'}</span></div>
    <div class="trow"><span class="tlabel">${t('lastMonth')}</span><span class="tval">${fmt(prevMoTotal)}</span></div>`;

  // Top cats for viewed month
  const catMap = {};
  monthly.forEach(e=>{catMap[e.cat]=(catMap[e.cat]||0)+e.amount;});
  ...
  document.getElementById('top-cats').innerHTML = sorted.length
    ? sorted.map((c,i) => {
        const pct=((catMap[c]/(monthTotal||1))*100).toFixed(1);
        return `<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:.5px solid var(--bdr);${i===sorted.length-1?'border-bottom:none':''}">
          ...
          <div style="font-family:'DM Mono',monospace;font-size:12px">${fmt(catMap[c])}</div>
        </div>`;
      }).join('')
    : `<div class="empty">${t('noData')}</div>`;

  // 6-month MoM chart (always relative to viewed month)
  const moTotals = [];
  for (let i=5; i>=0; i--) {
  ...
  momChartInst = new Chart(document.getElementById('mom-chart').getContext('2d'), {
    type:'bar',
    data:{labels:moTotals.map(m=>m.label),datasets:[{data:moTotals.map(m=>m.total),backgroundColor:moTotals.map((_,i)=>i===5?'#1D9E75':'#9FE1CB'),borderRadius:4,borderSkipped:false}]},
    options:{responsive:true,maintainAspectRatio:false,
      plugins:{legend:{display:false},tooltip:{callbacks:{label:ctx=>` ${fmt(ctx.raw)}`}}},
...
function renderYearly() {
  // Set titles safely (NO data-i18n on these elements — avoids inner span being wiped)
  ...
  yrChartInst = new Chart(document.getElementById('yr-chart').getContext('2d'), {
    type:'bar',
    data:{labels:MS,datasets:[{data:moTotals,backgroundColor:MS.map((_,i)=>i===thisM?'#1D9E75':'#9FE1CB'),borderRadius:4,borderSkipped:false}]},
    options:{responsive:true,maintainAspectRatio:false,
      plugins:{legend:{display:false},tooltip:{callbacks:{label:ctx=>` ${fmt(ctx.raw)}`}}},
      scales:{x:{grid:{display:false},ticks:{font:{size:11},color:'#888'}},
              y:{grid:{color:'rgba(0,0,0,0.05)'},ticks:{font:{size:10},color:'#888',callback:v=>'Rp'+Math.round(v/1000)+'k'}}}}
  ...
  const activeMo = moTotals.filter(v=>v>0);
  ...
  document.getElementById('yr-stats').innerHTML = `
    ...
    <div class="trow"><span class="tlabel">${t('peakMonth')}</span><span class="tval">${peakM>=0&&moTotals[peakM]>0?MS[peakM]:'—'}</span></div>
  ...
  const sortedC = Object.keys(catMap).sort((a,b)=>catMap[b]-catMap[a]);
  document.getElementById('yr-cat-breakdown').innerHTML = sortedC.length
    ? sortedC.map(c=>{
        const pct=((catMap[c]/yrTotal)*100).toFixed(1);
        return `<div class="bar-row"><div class="bar-top"><span>${catENtoDisplay(c)}</span>
          <span style="font-family:'DM Mono',monospace;color:var(--muted)">${fmt(catMap[c])} <span style="color:var(--hint)">${pct}%</span></span></div>
          <div class="bar-bg"><div class="bar-fill" style="width:${pct}%;background:${catENtoColor(c)}"></div></div></div>`;
      }).join('')
...
function buildCategorySelect() {
  ...
  oth.value = catKeyToEN('catOthers');
  oth.textContent = t('catOthers');
  sel.appendChild(oth);
...
async function importCSV(input) {
  ...
  reader.onload = async e => {
    ...
    for (let i=1; i<lines.length; i++) {
      ...
      const d  = new Date(ts);
      const exp = { id, name:fields[3]||fields[2], amount:parseFloat(fields[5]||fields[4])||0,
        cat:fields[4]||fields[3]||'Others', ts,
    ...
    m.textContent = `${t('importSuccess')} (${imported})`; m.style.display='block';
...
(async function init() {
  ...
  // Apply full language (renders everything)
  applyLanguage();
})();
